...
- `RocketMQ消息存储` 是 `RocketMQ` 体系中的一个具体知识点，建议先明确它解决的问题、参与的核心对象以及在整体链路中的位置。
...
- 回答 `RocketMQ消息存储` 时，可以按“定义 -> 原理/流程 -> 使用场景 -> 常见问题 -> 项目经验”的顺序展开。
- 如果不确定细节，优先讲清边界和排查思路，不要把不同组件或不同版本的行为混在一起。

### 复习检查

- 能用自己的话解释 `RocketMQ消息存储`，而不是只复述标题。
- 能说出至少一个生产场景、一个常见坑点和一个排查方向。

## 脑图解读

- 本节脑图围绕 `RocketMQ消息存储` 展开，属于 `RocketMQ` 的复习范围。
- 建议优先掌握：存储设计, 消息发送存储过程, 存储文件组织和内存映射。
- 二级节点提示了主要拆分角度：重点文件, 内存映射, 刷盘策略, transientStorePoolEnable, 文件恢复机制, 如果当前节点broker停止工作或当前不支持写入则拒绝写入, 如果消息的延迟级别大于0，则使用延时主题和队列, 获取当前可以写入的CommitLog, 写入CommitLog之前，先申请putMessageLock, 设置消息的存储时间，并判断CommitLog是否需要创建或可写入。
- 三级节点通常对应具体细节、API、机制或易错点：CommitLog, ConsumerQueue, Index, 在Java中使用FileChannel的map方法创建内存映射文件, 如果RocketMQ Broker进程异常退出，存储的页缓存中的数据并不会丢失，操作异同会董事将页缓存中的数据持久化到磁盘中，实现数据安全可靠。, 同步刷盘, 异步刷盘, 过程, 好处, 缺点。

### 复习追问

- `存储设计` 解决什么问题？核心机制是什么？有什么常见坑或替代方案？
- `消息发送存储过程` 解决什么问题？核心机制是什么？有什么常见坑或替代方案？
- `存储文件组织和内存映射` 解决什么问题？核心机制是什么？有什么常见坑或替代方案？
