...
- `Runner` 是 `Spring Boot` 体系中的一个具体知识点，建议先明确它解决的问题、参与的核心对象以及在整体链路中的位置。
...
- 回答 `Runner` 时，可以按“定义 -> 原理/流程 -> 使用场景 -> 常见问题 -> 项目经验”的顺序展开。
- 如果不确定细节，优先讲清边界和排查思路，不要把不同组件或不同版本的行为混在一起。

### 复习检查

- 能用自己的话解释 `Runner`，而不是只复述标题。
- 能说出至少一个生产场景、一个常见坑点和一个排查方向。

## 脑图解读

- 本节脑图围绕 `Runner` 展开，属于 `Spring Boot` 的复习范围。
- 建议优先掌握：介绍, 分类。
- 二级节点提示了主要拆分角度：启动时运行一些特定的代码, CommandLineRunner, ApplicationRunner。
- 三级节点通常对应具体细节、API、机制或易错点：获取启动命令行参数, 启动获取应用启动的时候参数。
