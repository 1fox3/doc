...
- `@Bean和@Component生成同名的Bean` 是 `Spring` 体系中的一个具体知识点，建议先明确它解决的问题、参与的核心对象以及在整体链路中的位置。
...
- 回答 `@Bean和@Component生成同名的Bean` 时，可以按“定义 -> 原理/流程 -> 使用场景 -> 常见问题 -> 项目经验”的顺序展开。
- 如果不确定细节，优先讲清边界和排查思路，不要把不同组件或不同版本的行为混在一起。

### 复习检查

- 能用自己的话解释 `@Bean和@Component生成同名的Bean`，而不是只复述标题。
- 能说出至少一个生产场景、一个常见坑点和一个排查方向。

## 脑图解读

- 本节脑图围绕 `@Bean和@Component生成同名的Bean` 展开，属于 `Spring` 的复习范围。
- 建议优先掌握：spring.main.allow-bean-definition-overriding。
- 二级节点提示了主要拆分角度：为true时，表示允许覆盖，以Spring后初始化的为准, 为false，表示不允许覆盖，抛异常。

### 复习追问

- `spring.main.allow-bean-definition-overriding` 解决什么问题？核心机制是什么？有什么常见坑或替代方案？
