...
- `Bean初始化进行操作,按执行顺序` 是 `Spring` 体系中的一个具体知识点，建议先明确它解决的问题、参与的核心对象以及在整体链路中的位置。
...
- 回答 `Bean初始化进行操作,按执行顺序` 时，可以按“定义 -> 原理/流程 -> 使用场景 -> 常见问题 -> 项目经验”的顺序展开。
- 如果不确定细节，优先讲清边界和排查思路，不要把不同组件或不同版本的行为混在一起。

### 复习检查

- 能用自己的话解释 `Bean初始化进行操作,按执行顺序`，而不是只复述标题。
- 能说出至少一个生产场景、一个常见坑点和一个排查方向。

## 脑图解读

- 本节脑图围绕 `Bean初始化进行操作,按执行顺序` 展开，属于 `Spring` 的复习范围。
- 建议优先掌握：@PostContrust, InitializingBean接口, initMethod方法（@Bean(initMethod="init")）。

### 复习追问

- `@PostContrust` 解决什么问题？核心机制是什么？有什么常见坑或替代方案？
- `InitializingBean接口` 解决什么问题？核心机制是什么？有什么常见坑或替代方案？
- `initMethod方法（@Bean(initMethod="init")）` 解决什么问题？核心机制是什么？有什么常见坑或替代方案？
