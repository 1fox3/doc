...
```bash
pip install langchain langchain-openai langchain-community langgraph langsmith langserve fastapi uvicorn deepagents python-dotenv pydantic
```
...
def need_ticket(state: CustomerServiceState) -> str:
    order = state["order"]
    if order.get("status") == "paid" and order.get("delivery") == "not_shipped":
        return "create_ticket"
    return "generate_answer"


def create_ticket(state: CustomerServiceState) -> CustomerServiceState:
    ticket = {
...
## Demo 8：LangGraph 工具调用 Agent

如果只是想快速构建一个带工具调用的 Agent，可以使用 LangGraph 预置的 `create_react_agent`。
...
agent = create_react_agent(
    llm,
    tools,
    prompt="你是电商客服助手。涉及订单状态时必须调用工具，不要编造。",
)

result = agent.invoke({
    "messages": [
        ("user", "帮我查一下订单 A1001 为什么还没发货，如果需要人工处理就创建工单。")
    ]
})
...
和前面的 `AgentExecutor` 相比，这个 demo 的底层执行也是一个图。LangGraph 会把模型节点、工具节点和是否继续执行的条件边组织起来。

生产建议：

1. 简单工具 Agent 可以用 `create_react_agent` 快速搭建。
2. 复杂业务流程建议手写 `StateGraph`，不要把所有控制权交给模型。
3. 高风险工具调用前插入人工确认节点。
4. 对每个节点写单元测试，而不是只测最终自然语言输出。
5. 保存关键状态，便于失败恢复和审计。
